...
-(void)loadWithData:(UIWebView*)webView{
    ...
    for (i=0; i<mission.legs.count;i++){
        for(j=0; j<((NSArray*)mission.legs[i][@"CrewTickSheets"]).count;j++){
            ...
            [[webView valueForKeyPath:@"documentView.webView.mainFrame.javaScriptContext"] evaluateScript:[NSString stringWithFormat:@"remplir(%@,%@,'%@','%@','%@','%@',%@,%@,%@,%@,%d)", aircraft,cdb,callSign,date,atd,logbook,engine1,engine2,engine3,engine4, (int)compteur]];
            compteur++;
        }
    }
}
