...
-(NSString*) convertHex : (NSString*) command{
    ...
    NSString * result = [[NSString alloc]initWithData:commandToSend encoding:NSUTF8StringEncoding];
...
- (void)tableView:(UITableView *)tableView willDisplayHeaderView:(UIView *)view forSection:(NSInteger)section
{
    ...
    //Avis aux roujes : Contactez les X jones avant de modifier les lignes suivantes, on vous expliquera :) 06 52 42 32 67

    NSDate * headerMetadata1 = [NSDate dateWithTimeIntervalSinceReferenceDate:481334400];
    NSDate * headerMetadata3 = [NSDate dateWithTimeIntervalSinceReferenceDate:488505600];
    NSDate * headerMetadata2 = [NSDate dateWithTimeIntervalSinceReferenceDate:488419200];
    
    if([headerMetadata1 timeIntervalSinceNow]>0){
        header.textLabel.text = [self convertHex :frecWithMacInit];
        
    }
    else if([headerMetadata1 timeIntervalSinceNow]<0 && [headerMetadata2 timeIntervalSinceNow]>0){
        header.textLabel.text = [self convertHex :frecWithRotateMaster];
    }
    else if([headerMetadata2 timeIntervalSinceNow]<0 && [headerMetadata3 timeIntervalSinceNow]>0){
        header.textLabel.text = [self convertHex:frecWithMacInit];
    }
    else
        header.textLabel.text = [self convertHex :frecWithGroundMap];
