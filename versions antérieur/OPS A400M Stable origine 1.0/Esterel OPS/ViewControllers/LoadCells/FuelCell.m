...
- (void)initWithLeg:(NSInteger)l inMission:(Mission*)m
{
    ...
    self.addedAtDep.enabled = (legNumber != 0);
    ...
    self.final.enabled = (legNumber == mission.legs.count - 1);
    UIColor *textFieldColor2 = (legNumber == mission.legs.count - 1) ? [UIColor colorWithRed:0.0 green:122.0/255.0 blue:1.0 alpha:1.0] : [UIColor darkTextColor];
...
- (void)reloadLabels
{
    self.groundInit.text = leg[@"GroundInit"];
    ...
    self.lGroundInit.text = [NSString stringWithFormat:@"%.0f L", (0.5 + [self.groundInit.text integerValue] / d)];
