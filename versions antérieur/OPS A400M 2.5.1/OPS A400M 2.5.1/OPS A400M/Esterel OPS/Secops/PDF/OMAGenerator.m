...
- (void)printPDF
{
    ...
    self.PDFCreator = [NDHTMLtoPDF createPDFWithHTML:html baseURL:[[NSBundle mainBundle] resourceURL] pathForPDF:[[[@"~/Documents/OMA_" stringByAppendingString: mission.legs.firstObject[@"CallSign"]] stringByAppendingString:@".pdf"] stringByExpandingTildeInPath] pageSize:fPaperSizeA4 margins:UIEdgeInsetsMake(10, 5, 10, 5) successBlock:^(NDHTMLtoPDF *htmlToPDF) {
...
-(void)loadWithData:(UIWebView *)webView
{
   ...
   int k=1;
    ...
    for(NSDictionary *leg in legs){
        ...
        //Night35
        if(![leg[@"Night35"] isEqualToDate:dateNulle])
        ...
        if(![leg[@"Night22"] isEqualToDate:dateNulle])
        ...
        [ligneCrew appendFormat:@",'%@ <br/> <span class=\"brownSpan\">%@</span>  <span style=\"display: none;\" class=\"blueSpan\">%@</span>'", [TimeTools stringFromDate:leg[@"OffBlocksTime"]], [TimeTools stringFromTime:leg[@"OffBlocksTime"] withDays:NO], [TimeTools stringFromTime:leg[@"TakeOffTime"] withDays:NO]];
        //OnBlocksTime with Landing time    //X15 modified//
        [ligneCrew appendFormat:@",'%@ <br/> <span style=\"display: none;\" class=\"blueSpan\">%@</span> <span class=\"brownSpan\">%@</span>'", [TimeTools stringFromDate:leg[@"OnBlocksTime"]], [TimeTools stringFromTime:leg[@"LandingTime"] withDays:NO], [TimeTools stringFromTime:leg[@"OnBlocksTime"] withDays:NO]];
        //Day10
        
        if(![leg[@"Day10"] isEqualToDate:dateNulle])
        ...
        if(![leg[@"Day62"] isEqualToDate:dateNulle])
        ...
        //Fuel Burned
        [ligneCrew appendFormat:@",'%d'", [leg[@"FuelBurned"] integerValue]];
        ...
        //On a rajouté ces trois dernières lignes. Le esle ne sert à rien mais il était là avant...(s)X2015
    ...
    *totDay = [NSDate dateWithTimeIntervalSinceReferenceDate:totalJour],
    *totNight = [NSDate dateWithTimeIntervalSinceReferenceDate:totalNuit];
    
    [moncontext evaluateScript:[NSString stringWithFormat:@"var x = new Array('%@', '%@', '%@', '%@', '%@', '%@', '%@', '%@' , '%@', '%@' ); addTotalHours(x);",
                                [TimeTools stringFromTime:totDay withDays:YES],
                                [TimeTools stringFromTime:totNight withDays:YES],
