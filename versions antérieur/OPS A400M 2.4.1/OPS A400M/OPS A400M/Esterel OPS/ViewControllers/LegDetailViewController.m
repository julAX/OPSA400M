...
@synthesize hideShowText;
@synthesize omaCell,ctsCell,omaTitle,ctsTitle;
...
- (NSInteger) tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section {
    if(section == 2)
        ...
        return (((SplitViewController* )self.splitViewController).showDetLeg)?4:1;
    else if(section==4)
        return (((SplitViewController* )self.splitViewController).showTools)?3:0;
    else if(section==3)
        return (((SplitViewController* )self.splitViewController).showTools)?10:3;
    else
        return 0;
}
...
-(UITableViewCell*)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if(indexPath.section!=1 && indexPath.section !=2){
        ...
        cell.contentView.backgroundColor=transparant;
    }
...
- (void)reloadColors
{
    ...
    aircraft.textColor = blackColor;
    missionNumber.textColor = blackColor;
    callSign.textColor = blackColor;
    be.textColor =  blackColor;
    na.textColor =  blackColor;
    
    // Partie de indicators
    
    NSDictionary * indic = leg[@"Indicators"];
    
    ctsCell.backgroundColor = ([indic[@"CrewTickSheet"][1] isEqualToString:@"1"])? vert : ( ([indic[@"CrewTickSheet"][0] isEqualToString:@"1"])? jone2 : rouge );
    omaCell.backgroundColor = ([indic[@"OMA"][1] isEqualToString:@"1"])? vert : ( ([indic[@"OMA"][0] isEqualToString:@"1"])? jone2 : rouge );
    ctsCell.contentView.backgroundColor = transparant;
    omaCell.contentView.backgroundColor = transparant;
...
- (NSString *)tableView:(UITableView *)tableView titleForHeaderInSection:(NSInteger)section
{
    ...
    if(section==0 || section==1)
        ...
        return @"Easy Crew Tick Sheet";
    
    else if (section==3)
        return (((SplitViewController*)self.splitViewController).showTools)? [NSString stringWithFormat: @"Leg %ld detailled infos",  (long)(legNumber + 1) ] : [NSString stringWithFormat: @"Leg %ld detailled infos and tools",  (long)(legNumber + 1)];
    else
        return @"More Tools";
...
- (CGFloat)tableView:(UITableView *)tableView heightForHeaderInSection:(NSInteger)section
{
    if(section!=4){
        ...
        return (!((SplitViewController*)self.splitViewController).showTools)? 0 : [super tableView:tableView heightForHeaderInSection:section];
...
- (IBAction)hideButton:(id)sender {
    ...
    if(((UIButton*)sender).tag==0){
        ((SplitViewController*)self.splitViewController).showDetLeg = ! ((SplitViewController*)self.splitViewController).showDetLeg;
        [self.tableView reloadData];
        }
    else{
        ((SplitViewController*)self.splitViewController).showTools = ! ((SplitViewController*)self.splitViewController).showTools;
        [self.tableView reloadData];
    }
}
@end
