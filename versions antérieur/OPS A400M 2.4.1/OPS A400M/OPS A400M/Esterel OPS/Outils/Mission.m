...
- (Mission *)initWithFile:(NSString *)path
{
    ...
    for(NSMutableDictionary *leg in self.legs){
        ...
        if(!leg[@"CrewTickSheets"]){
            ...
            NSMutableArray *engine = [[NSMutableArray alloc] initWithArray:@[@[@"0",datenulle],@"1",@"0",@"0",@"0",@[@"0",datenulle],@"0",@"0",@"0",@"0",datenulle]];
...
- (void)save
{
    ...
    while(carg = [instancesEnumerator nextObject]){
        ...
        cargDict[@"nombrePax"]=@(carg.nombrePax).description;
...
-(void) indicatorsForLeg:(NSInteger)l{
    ...
    if([indic[@"PaxAndFreight"][1]isEqualToString:@"0"])
        omafinished = NO;
    ...
    if([indic[@"OMA"][1] isEqualToString:@"0"] || [indic[@"CrewTickSheet"][1] isEqualToString:@"0"])
        legFinished=NO;
    if([indic[@"OMA"][0] isEqualToString:@"1"] || [indic[@"CrewTickSheet"][0] isEqualToString:@"1"])
        legInProgress=YES;
