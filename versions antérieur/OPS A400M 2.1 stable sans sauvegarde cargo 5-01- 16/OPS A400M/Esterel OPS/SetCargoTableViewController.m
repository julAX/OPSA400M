...
- (IBAction)buttonDepartureAirport:(id)sender {
    ...
    //currentCargo.numArrnumDep[0]=[NSNumber numberWithInt:(index.row-1)];
...
- (IBAction)buttonArrivalAirport:(id)sender {
    ...
    NSInteger longueurListe = airportList.count;
    //NSIndexPath * index = [sender con];
    
    //currentCargo.numArrnumDep[1]=[NSNumber numberWithInt:(index.row-1)];
    for (NSInteger j = 1; j<longueurListe; j++){
        NSIndexPath *path = [NSIndexPath indexPathForRow:(NSInteger)j inSection:1];
        AirportTableViewCell * currentCell = (AirportTableViewCell*)[myTableView cellForRowAtIndexPath:path];
...
- (IBAction)enterWeightButton:(id)sender {
    ...
    if(currentCargo.be.length!=0 && currentCargo.arrival && currentCargo.departure /*&& currentCargo.numArrnumDep[1]>=currentCargo.numArrnumDep[0]*/){
    ...
    //NSIndexPath * index = [sender indexPath];
    //[myTableView cellForRowAtIndexPath:index].backgroundColor = [UIColor redColor];
    }
}
