    ...
    if (indexPath.section == 0)
    {
        ...
        switch (indexPath.row) {
                    ...
                    UIAlertAction *sendAction = [UIAlertAction
                                               ...
                                               handler:^(UIAlertAction *action1)
                                               {
                                                   if (![MFMailComposeViewController canSendMail]) {
                                                       ...
                                                       [composeVC setMessageBody:[NSString  stringWithFormat: @"Bonjour,\n\nVeuillez recevoir l'OMA rempli non signé ainsi que le referentiel entrainement pour le %@. \n\n %@",mission.legs.firstObject[@"CallSign"],[mission cdbForLeg:0]] isHTML:NO];
            ...
            case 3:{
                ...
                BOOL problem = NO;
                for (NSMutableDictionary *leg in mission.legs){
                    if([leg[@"Engine"][0] isEqualToString:@""])
                        problem = YES;
                }
                
                if(problem){
                    UIAlertController *alertController = [UIAlertController
                                                          alertControllerWithTitle:@"Attention Crew Tick Sheet !"
                                                          message:@"Some crew tick sheets don't have a LOGBOOK N° !"
                                                          preferredStyle:UIAlertControllerStyleAlert];
                    ...
                    UIAlertAction *okAction = [UIAlertAction
                                               actionWithTitle:NSLocalizedString(@"Continue to preview", @"OK action")
                                               style:UIAlertActionStyleDefault
                    ...
                    UIAlertAction *sendAction = [UIAlertAction
                                                 ...
                                                 handler:^(UIAlertAction *action1)
                                                 {
                                                     if (![MFMailComposeViewController canSendMail]) {
                                                         ...
                                                         [composeVC setMessageBody:[NSString  stringWithFormat: @"Bonjour,\n\nVeuillez recevoir les crew tick sheets remplies pour le %@. \n\n %@",mission.legs.firstObject[@"CallSign"],[mission cdbForLeg:0]] isHTML:NO];
