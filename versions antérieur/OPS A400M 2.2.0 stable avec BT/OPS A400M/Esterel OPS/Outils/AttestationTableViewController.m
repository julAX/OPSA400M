...
- (void)viewWillAppear:(BOOL)animated
{
    ...
    [self setEtape:0 forLabel:debutLabel];
    [self setEtape:0 forLabel:finLabel];
}
...
- (IBAction)etapeChanged:(UIStepper *)sender {
    ...
    if (sender == debutStepper) {
        ...
        [self setEtape:debutStepper.value forLabel:debutLabel];
        
        if (debutStepper.value > finStepper.value) {
            finStepper.value = debutStepper.value;
            [self setEtape:finStepper.value forLabel:finLabel];
        }
    }
...
- (void)setEtape:(double)d forLabel:(UILabel*)label
{
    NSInteger i = round(d);
        ...
        if ([self genererPdf]) {
            ...
            switch (indexPath.row) {
                ...
                case 1:
                    ...
                    NSLog(@"%@",pdfUrl.description);
                    NSLog(@"%@",documentController.description);
...
- (BOOL)genererPdf
{
    ...
    NSString *avion = @{@"FRB-AA" : @"A400M MSN7",
                        @"FRB-AB" : @"A400M MSN8",
                        @"FRB-AC" : @"A400M MSN10",
                        @"FRB-AD" : @"A400M MSN11",
                        @"FRB-AE" : @"A400M MSN12",
                        @"FRB-AF" : @"A400M MSN14"}[mission.root[@"Aircraft"]], *text;
    ...
    for (int l = round(debutStepper.value); l <= dernierLeg; l++) {
        ...
        for (NSString *coeff in @[@"35", @"22", @"10", @"62"]) {
            ...
            if (!([jour isEqualToDate:timeZero] && [nuit isEqualToDate:timeZero])) {
                ...
                {
                    if ([leg[@"DayOtherCode"] isEqualToString:leg[@"NightOtherCode"]])
                        [lignes addObject:@[date, fonction.text, avion, trajet, atterissages, jour, nuit, leg[@"DayOtherCode"], dateFin]];
